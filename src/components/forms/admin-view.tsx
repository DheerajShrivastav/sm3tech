...
import { getAgencies, getUser, getPlanApprovals, getStabilityCertificates, getSafetyAuditReports, getConsentToEstablishDocuments, getConsentToOperateDocuments, getComplianceReports } from '@/lib/queries'
...
const AdminView = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (user?.role === 'Admin') {
          const [agenciesResponse, planApprovalsResponse, stabilityCertificatesResponse, safetyAuditReportsResponse, consentToEstablishResponse, consentToOperateResponse, complianceReportsResponse] = await Promise.all([
            getAgencies(),
            getPlanApprovals(),
            getStabilityCertificates(),
            getSafetyAuditReports(),
            getConsentToEstablishDocuments(),
            getConsentToOperateDocuments(),
          ...
          setConsentToOperateDocs(consentToOperateResponse)
          setComplianceReports(complianceReportsResponse)
        }
      } catch (error) {
  ...
  const renderConsentToOperate = () => {
    return (
      <Table className="w-full border border-gray-200 rounded-lg shadow-sm font-sora">
        <TableCaption className="text-left text-gray-600 font-light">
          List of Consent To Operate Documents
        </TableCaption>
        <TableHeader>
          <TableRow className="bg-gray-100">
            <TableHead className="py-3 px-4 font-medium text-gray-700">User Email</TableHead>
        ...
        <TableBody>
          {(consentToOperateDocs as IConsentToOperate[]).map((doc) => (
            <TableRow key={doc._id as Key} className="border-t">
              <TableCell className="py-4 px-4 font-medium text-gray-900">
                {(doc.user as any)?.email || 'N/A'}
              </TableCell>
  ...
  const renderComplianceReports = () => {
    return (
      <Table className="w-full border border-gray-200 rounded-lg shadow-sm font-sora">
        <TableCaption className="text-left text-gray-600 font-light">List of Compliance Reports</TableCaption>
        <TableHeader>
          <TableRow className="bg-gray-100">
            <TableHead className="py-3 px-4 font-medium text-gray-700">User Email</TableHead>
            <TableHead className="py-3 px-4 font-medium text-gray-700">Status</TableHead>
        ...
        <TableBody>
          {(complianceReports as IComplianceReport[]).map((report) => (
            <TableRow key={report._id as Key} className="border-t">
              <TableCell className="py-4 px-4 font-medium text-gray-900">
                {(report.user as any)?.email || 'N/A'}
              </TableCell>
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      ...
      <div className="bg-white shadow-lg rounded-lg p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Plan Approvals</h2>
        {renderPlanApprovals()}
      </div>
      <div className="bg-white shadow-lg rounded-lg p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Stability Certificates</h2>
        {renderStabilityCertificates()}
      </div>
      <div className="bg-white shadow-lg rounded-lg p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Safety Audit Reports</h2>
        {renderSafetyAuditReports()}
      </div>
      <div className="bg-white shadow-lg rounded-lg p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Consent To Establish</h2>
        {renderConsentToEstablish()}
      </div>
      <div className="bg-white shadow-lg rounded-lg p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Consent To Operate</h2>
        {renderConsentToOperate()}
      </div>
      <div className="bg-white shadow-lg rounded-lg p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Compliance Reports</h2>
